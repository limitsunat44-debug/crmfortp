...
## UI/UX редизайн + расширенные профили врачей (sql-doctor-profiles-ui.sql)

### Визуальный стиль
Интерфейс переоформлен в стиле современного медицинского приложения:
светлый серо-белый фон, teal/green акцент, крупные скруглённые карточки,
мягкие тени, чипы для категорий/статусов, скруглённый поиск. Однофайловая
архитектура `index.html` сохранена.

- **Навигация**: на десктопе — вкладки-чипы в шапке; на мобильном (≤900px) —
  фиксированная нижняя навигация (bottom nav), как в приложении.
- **Таблицы**: на узких экранах (≤640px) превращаются в карточки (card/table
  hybrid) с подписями колонок через `data-label`.

### Вкладка «Врачи»
- Грид карточек врачей с аватаром-инициалом, специальностью-чипом, описанием,
  рейтингом и телефоном.
- Скруглённый поиск (имя/код/место работы/специальность/телефон).
- Фильтр по специальности через чипы.
- Модалка профиля врача: имя, код, специальность, телефон, место работы,
  номер кошелька, рейтинги (профессионализм / сотрудничество, 1–5 звёзд),
  описание, заметки. Сохранение в `crm_doctors` (insert при `source='manual'`,
  patch для существующих).
- CRUD специальностей (`crm_doctor_specialties`): добавить/удалить, иконка-эмодзи.

### Быстрые периоды на вкладке «Продажи»
Чипы: **Май 2026**, Текущий месяц, Прошлый месяц, Последняя синхронизация.
По умолчанию вкладка открывается на последней синхронизации, иначе на Мае 2026.

### Миграция `sql-doctor-profiles-ui.sql`
Идемпотентная (повторный запуск безопасен). Применять **после**
`sql-sales-plan-payments.sql`.

- `ALTER TABLE crm_doctors ADD COLUMN IF NOT EXISTS ...`:
  `description`, `workplace`, `phone`, `wallet_number`, `specialty`,
  `specialty_id`, `professional_rating` (1–5), `cooperation_rating` (1–5),
  `notes`. Рейтинги ограничены CHECK 1..5.
- `CREATE TABLE IF NOT EXISTS crm_doctor_specialties` (name/icon/color/sort_order)
  + стартовый набор специальностей (вставляется, только если таблица пуста).
- FK `crm_doctors.specialty_id → crm_doctor_specialties.id` (ON DELETE SET NULL).
- RLS-политики в стиле проекта (anon read+write) для `crm_doctor_specialties`.

**Важно про синхронизацию:** `scripts/sync-sales.js` делает upsert по `doctor_code`
и пишет только sync-поля (`doctor_name`, `discount_card_ref`, `discount_card_code`,
`source`, `discount_group`, `active`, `last_synced_at`). Ручные поля профиля в upsert
не входят, поэтому `resolution=merge-duplicates` их **не затирает**. Менять
`sync-sales.js` не требуется.
